...
# MAGIC
# MAGIC In order to do this we first use the [**groupby([columns])**](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.groupby.html) method and specify the category we want to group by. In this case, let's group by **`neighborhood`**.
